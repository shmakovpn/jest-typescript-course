...
Есть такая парадигма *Test Driven Development* (`TDD`).
Сначала мы пишем тесты, а потом потом код.
Такой подход еще называют *Экстремальным программированием*.
Кента Бек даже книгу написал, которая так и называется *Экстремальное программирование*.
...
it('length', () => { // it тоже самое, что test, только короче
  expect('foo'.length).toEqual(3);
});
```

Нажимаю Ctrl+Shift+P и выбираю команду *Jest: Run all tests*.
...
Есть такой прием *триангуляция*, когда для выяснения координат объекта,
...
В отличии от геометрии, таких *триангуляций* может быть бесчисленное множество.
