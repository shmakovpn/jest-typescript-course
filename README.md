...
Конечно в `tsconfig.json` еще много параметров, и в рамках данной книги я не ставлю себе цель 
объяснить их все, т.к. даже из коробки все прекрасно работает. 
Всему свое время. Уверяю вас, вы без проблем (ну почти) научитесь настраивать компилятор. 
Во многих крутых framework-ах (например `Angular` или `Nest`) есть специальные программы, мастера создания новых проектов.
Они сами генерят содержимое `tsconfig.json`. Так, что не стоит паниковать если сейчас вы чувствуете, 
что плаваете в данной теме. Смело экспериментируйте. 

"Что значит экспериментируйте? 
Тебе легко говорить, мы еще на ногах стоять не научились, а ты призываешь нас походить, побегать" - спросите вы. 
И прежде чем закончить тему `tsconfig.json`, я возьму вас за ручку и мы сделаем этот шаг вместе.
Поставим эксперимент. 
Включим `линтеры` компилятора на максимум, сделаем так, 
чтобы `tsc` наиболее строго проверял наш код.
**Обязательно загуглите, что такое `линтеры` каждый крутой программист должен знать это слово**.
Но, вернемся к теме.
`typescript` основан на `javascript`, в котором через чур много вольностей, а природа человека такова, 
что людям свойственно ошибаться, не замечая свои ошибки при этом. 
Чем строже компилятор проверяет наш код, тем выше вероятность выявить в нем ошибку на самом раннем этапе.
Напомню, что основной посыл данной книги, это не только писать код, но и всячески способствовать тому, 
чтобы сам компьютер его максимально эффективно проверял.
Хотя, настройка компилятора и не имеет прямого отношения к тестам, 
все же это еще один из способов заставить компьютер помогать нам. 
Обратите внимание на секцию `Type Checking`, она отвечает за глубину проверки . 
