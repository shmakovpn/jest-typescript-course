...
К чему все это я? 
Наша программа всего лишь выводит в консоль фразу *Hello World from typescript*.
...
Она запускает его `$(node dist)` и сравнивает вывод со строкой *Hello World from typescript*.
Если строки совпадают, будет выведено *Все работает*, если не совпадают *Что-то пошло не так*.
...
Поведение программы изменилось, строки перестали совпадать и *что-то пошло не так*.
...
Как правило программы сложнее чем *Hello world*. 
...
Напишем частичный тест, который делает больше ничего не делает, только import.
...
![tsc-index-spec-import](docs/images/tsc-index-spec-import.png)

Охота не удалась. 
Внимательный читатель наверное заметил как VSCode заботливо указывает на проблему и даже предлагает решение.
